...
class Player(Cards):
    ...
    def judge(self, legal_now=Legal(), num=1, brain_ind=0, at=0):
...
def predict(func, inpu, at):
    Situation.queue.append((func, inpu))
    Situation.rec_pos.append((at, Situation.at_num[at]))
    Situation.at_num[at] += 1


def implement():
    for i in range(len(Situation.queue)):
        Situation.queue[i] = Situation.queue[i][0](Situation.queue[i][1])
